...
======================================================================================= */
	...
	/* 1A. Update the navigation bar on click
	----------------------------------------------------------------------------------- */  	
	
	$('#main-nav a, #intro-nav a').live('click', function() {
	...
	/* 1C. Update navigation and do stuff in sections when scrolling down
	----------------------------------------------------------------------------------- */  	
	...
	$('.section').waypoint(function(direction) 
		{
    		if (direction === 'down') {
    			...
    			// show or hide services background
    			if ((id == 'prologue') || ($(this).index() == 1)) {
    				$('#services-bg').hide();
    			}
    			else {
    				$('#services-bg').show();
...
======================================================================================= */
	...
	/* 4B. Click action of radio buttons or checkboxes
	----------------------------------------------------------------------------------- */ 
...
======================================================================================= */
	...
	function loadProject(project, titleOfProject, hrefOfProject) {
		...
		if (project.is(':last-child')) {
			...
			$('#project-nav').find('#controls').html('<li><a class="close" href="#"><span>Close</span></a></li><li><a class="prev" href="'+prevLink+'"><span>Prev</span></a></li>');
		}
		
		if (project.is(':first-child')) {
			var nextLink = project.next().find('a').attr('href');
			$('#project-nav').find('#controls').html('<li><a class="close" href="#"><span>Close</span></a></li><li><a class="next" href="'+nextLink+'"><span>Next</span></a></li>');
		}

		if (project.is(':not(:first-child)') && project.is(':not(:last-child)')) {
			var prevLink = project.prev().find('a').attr('href');
			var nextLink = project.next().find('a').attr('href');
			$('#project-nav').find('#controls').html('<li><a class="close" href="#"><span>Close</span></a></li><li><a class="next" href="'+nextLink+'"><span>Next</span></a></li><li><a class="prev" href="'+prevLink+'"><span>Prev</span></a></li>');
		}	   
		...
		showProject = setTimeout(function() {
    		...
    		$('#project').load(hrefOfProject, function() {
				...
				$('.pslides li').mousemove(function(e){
					...
					$(this).find('img').animate({marginLeft: '-' + myX + 'px', marginTop: '-' + myY + 'px'},{duration: 100, queue: false, easing: 'linear'});
				});					
			}); 		
    	},1000);
	}

	/* 6C. Function Timeline
	----------------------------------------------------------------------------------- */
